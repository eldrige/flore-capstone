...
const BlogLandingPage = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (!token) {
        console.log(
          'No authentication token found, using default profile image'
        );
        setIsLoading(false);
        return;
      }
  ...
  const fetchBlogPosts = async () => {
    try {
      const response = await fetch('http://3.82.241.188/api/blog-posts');
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://3.82.241.188/api/newsletter/subscribe',
        { email }
      );
    ...
    <div className="min-h-screen flex flex-col justify-between px-4 md:px-8 py-6 bg-gray-50">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Advanced Assessment Tools and Features for Skill-Driven Learners
            </p>
          </div>
            ...
            {isLoading ? (
              ...
              <>
                {getPaginatedPosts().map((post) => (
                  <article
                    key={post.id}
                    className="flex flex-col md:flex-row gap-4 mb-6 bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow"
                  >
      ...
      <section className="bg-gradient-to-r from-green-50 to-green-100 p-6 md:p-8 text-center rounded-lg mt-8 shadow-inner">
        ...
        <form
          ...
          className="mt-4 flex flex-col md:flex-row justify-center max-w-md mx-auto"
        >
          <input
            type="email"
        ...
        {emailStatus && (
          <p
            className={`text-sm mt-2 ${
              emailStatus.includes('valid') ? 'text-red-600' : 'text-green-700'
            }`}
          >
            {emailStatus}
          </p>
      ...
      <footer className="bg-white shadow-inner text-center p-8 mt-8 rounded-lg">
        ...
        <div className="max-w-md mx-auto mt-2">
          <p className="text-sm text-gray-600">
            &copy; 2023 SkillsAssess. All rights reserved.
          </p>
